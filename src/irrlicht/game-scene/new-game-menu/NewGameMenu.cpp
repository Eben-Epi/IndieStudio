...
Irrlicht::NewGameMenu::NewGameMenu(Screen &screen, const std::string &name, unsigned id) :
        GameScene(screen, name, id), _playersNumber(1), _soundVolume(5), _tmpSoundVolume(5)
{
    //TITLE
    ...
    this->_buttons.emplace_back(new Button({370, 200}, {20, 120, 110, 120 + 32}, NUMBER_OF_AI_LESS, this->_window.getGuiEnv(), "LESS"));
            ...
            switch (i) {
                ...
                case SOUND_LESS:
                    if (this->_soundVolume >= 0) {
                        this->_soundVolume++;
                        this->_textBoxes[4]->setText(std::to_string(this->_playersNumber));
                    }
                    break;
                case SOUND_MORE:
